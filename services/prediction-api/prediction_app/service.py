...
class PredictionService:
    ...
    def run_prediction_c(self, inputs: schemas.PredictionInputC) -> schemas.PredictionOutput:
        ...
        input_summary = {
            "Substrate Size": f"{inputs.substrate}x{inputs.substrate} (mm²)",
            "Copper Ratio": f"{inputs.copper} (%)",
            "Magnet Count": inputs.magnet,
            "Jig Thickness": f"{inputs.jig:.3f} (mm)",
            "Jig Center Hole": f"{inputs.b1}x{inputs.w1} (mm²)",
            "Tool Height": f"{inputs.tool_height:.4f} (mm)"
        }
